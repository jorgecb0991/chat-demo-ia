  ...
  const generarSlides = async () => {
    ...
    try {
      const payload: any = {
        ...
        instruction_teacher: instruccionesProfesor.trim() || undefined,
      };
  
      // 👉 Si es archivo, incluimos file_bytes y mime_type
      if (sourceType === "file") {
        if (!fileBytes || !mimeType) {
          setLoading(false);
          return alert("Debes subir un archivo");
        }
        payload.file_bytes = Array.from(fileBytes); // lo mandamos como arreglo de números
        payload.mime_type = mimeType;
        payload.sourceValue = "archivo_subido"; // dummy, solo para cumplir backend
      }
      ...
      const response = await fetch("/api/ppt/generate-ppt", {
        ...
        body: JSON.stringify({
          ...
          numSlides,
          plantilla: plantillaSeleccionada,
          instrucciones_profesor: instruccionesProfesor.trim() || undefined
        }),
