...
export default function SlideGeneratorPage() {
  ...
  const [resultado, setResultado] = useState<EstructuraSlide | null>(null);
    ...
    // Cargar plantillas disponibles al montar el componente
  useEffect(() => {
  ...
  const generarSlides = async () => {
    ...
    try {
      const response = await fetch("/api/bedrock-generate-slide", {
        method: "POST",
      ...
      setResultado(data.estructura);
    } catch (error) {
      console.error("Error al generar slides:", error);
      alert("Ocurrió un error al generar los slides");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      ...
      {resultado && (
        <div className="mt-6 border-t pt-4 space-y-4">
          ...
          <p className="font-medium text-lg">{resultado.titulo}</p>
          <ul className="space-y-3">
            {resultado.slides.map((slide, idx) => (
              <li key={idx} className="bg-black p-4 rounded">
